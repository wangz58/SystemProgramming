...
Vector Vector::operator+(const Vector &other) const {
  return Vector(values_[0] + other.values_[0], values_[1] + other.values_[1], values_[2] + other.values_[2]);
}
...
double Vector::operator*(const Vector &other) const {
  return values_[0]*other.values_[0] + values_[1]*other.values_[1] + values_[2]*other.values_[2];
}

// scalar multiplication: if this is (a,b,c), return (ak,bk,ck)
Vector Vector::operator*(const double k) const {
  return Vector(values_[0] * k, values_[1] * k, values_[2] * k);
}
...
std::ostream & operator<<(std::ostream &out, const Vector &v) {
  out << "(" << v.values_[0] << "," << v.values_[1] << "," << v.values_[2] << ")";
  return out;
}
}
